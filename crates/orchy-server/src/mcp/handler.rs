...
impl OrchyHandler {
    ...
    pub(crate) fn get_session_namespace(&self) -> Option<Namespace> {
        self.session.read().unwrap().as_ref().map(|s| s.namespace.clone())
    }
    ...
    pub(crate) fn require_session(&self) -> Result<(AgentId, Namespace), String> {
        ...
        match guard.as_ref() {
            ...
            None => Err("no agent registered for this session; call register_agent first".to_string()),
        }
    }

    pub(crate) fn set_session(&self, agent_id: AgentId, namespace: Namespace) {
        *self.session.write().unwrap() = Some(SessionState { agent_id, namespace });
    }
    ...
    pub(crate) fn resolve_namespace(&self, explicit: Option<&str>) -> Result<Namespace, String> {
        match explicit {
            Some(ns_str) => {
                ...
                let session_ns = self.get_session_namespace()
                ...
                if ns.project() != session_ns.project() {
                    return Err(format!(
                        ...
                        ns, session_ns.project()
                    ));
                }
                Ok(ns)
            }
            None => {
                self.get_session_namespace()
