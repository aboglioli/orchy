...
impl OrchyHandler {
    ...
    async fn save_context(
        ...
        match self
            .container
            .knowledge_service
            .save_context(project, agent_id, namespace, params.summary, metadata)
            .await
        {
    ...
    #[tool(
        description = "Load the latest context snapshot. Defaults to the current agent's context."
    )]
    async fn load_context(
        ...
        let agent_id = match params.agent_id.as_deref() {
            Some(id_str) => parse_agent_id(id_str)?,
            None => session_agent,
        };
