...
pub async fn run(
    ...
    match cmd {
        MessageSubcommand::Send {
            ...
            let mut body_json = serde_json::json!({ "to": to, "body": body });
        ...
        MessageSubcommand::Inbox { limit } => {
            ...
            let v = client
                .get_project_json(&format!("/agents/{agent_id}/inbox{query}"))
                .await?;
            if config.json {
                output::print_json(config, &v);
            } else {
        ...
        MessageSubcommand::Sent { limit } => {
            ...
            let agent_id = client
                ...
                .ok_or(crate::client::CliError::MissingAgentId)?;
            let v = client
                .get_project_json(&format!("/agents/{agent_id}/sent-messages{query}"))
                .await?;
            if config.json {
                output::print_json(config, &v);
            } else {
        ...
        MessageSubcommand::MarkRead { ids } => {
            ...
            let agent_id = client
                ...
                .ok_or(crate::client::CliError::MissingAgentId)?;
            let v = client
                .post_project_json(&format!("/agents/{agent_id}/messages/read"), Some(&body))
                .await?;
            if config.json {
                output::print_json(config, &v);
            } else {
