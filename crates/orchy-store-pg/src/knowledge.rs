...
impl KnowledgeStore for PgKnowledgeStore {
        ...
        if let Some(pv) = entry.persisted_version() {
            ...
            if result.rows_affected() == 0 {
                let stored_version: Option<i64> = sqlx::query_scalar(
                    "SELECT version FROM knowledge_entries WHERE archived_at IS NULL AND id = $1",
                )
                .bind(entry.id().as_uuid())
    ...
    async fn list(&self, filter: KnowledgeFilter, page: PageParams) -> Result<Page<Knowledge>> {
        ...
        if !filter.include_archived.unwrap_or(false) {
            select.and_where(Expr::col(KnowledgeEntries::ArchivedAt).is_null());
        }
...
async fn search_by_embedding(
    ...
    let mut sql = format!(
        "SELECT {SELECT_COLUMNS}, (1.0 - (embedding <=> $1)) AS score FROM knowledge_entries WHERE embedding IS NOT NULL AND organization_id = $2 AND archived_at IS NULL"
    );
